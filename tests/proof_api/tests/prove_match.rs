...
#[test]
fn test_prove_equality_from_match() {
    let mut egraph = EGraph::with_proofs();
    ...
    // We should find both sum1 and sum2
    ...
    if matches.len() >= 2 {
        // Get the s values from two different matches
        let s1 = matches[0].get("s").unwrap();
        let s2 = matches[1].get("s").unwrap();

        // These might be equal due to the commutative rule
        // Try to prove they are equal
        if let Ok(proof) = egraph.explain_terms_equal(s1.clone(), s2.clone(), &mut store) {
