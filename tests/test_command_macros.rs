    ...
    fn transform(
        ...
        match command {
            Command::Rule { rule } => {
                ...
                Ok(vec![Command::Rule { rule: rule1 }, Command::Rule { rule: rule2 }])
            }
            cmd => Ok(vec![cmd]),
        }
    }
...
fn test_single_macro_with_desugar_program() {
    ...
    egraph.command_macros_mut().register(Arc::new(PrefixRuleMacro {
    ...
    let result = egraph.desugar_program(None, input).unwrap();
    ...
    assert!(output.contains("(function Num (i64) Math :cost 1)"), "Expected Num constructor");
    ...
    assert!(output.contains("test_$rule_"), "Expected rule name to be prefixed with test_: {}", output);
...
fn test_multiple_macros_compose_with_desugar_program() {
    ...
    assert!(output.contains("second_first_$rule_"), 
...
fn test_duplicate_macro_creates_two_rules() {
    let mut egraph = EGraph::default();
    egraph.command_macros_mut().register(Arc::new(DuplicateRuleMacro));
    ...
    assert!(output.contains("dup1_$rule_"), "Expected first duplicated rule: {}", output);
    assert!(output.contains("dup2_$rule_"), "Expected second duplicated rule: {}", output);
...
fn test_macro_adds_commands_after_rules() {
    let mut egraph = EGraph::default();
    egraph.command_macros_mut().register(Arc::new(CommentAfterRuleMacro));
    ...
    assert!(output.contains("(print-size rule_$rule_"), 
            "Expected print-size marker after rule: {}", output);
    
    // The let should still be there, unaffected
    assert!(output.contains("(let a (Num 1))"), "Expected let statement: {}", output);
}
...
fn test_complex_macro_composition() {
    ...
    assert!(output.contains("prefixed_dup1_$rule_"), 
            "Expected first rule with both transformations: {}", output);
    assert!(output.contains("prefixed_dup2_$rule_"), 
...
fn test_macros_work_with_actual_program_execution() {
    ...
    assert!(result.is_ok(), "Program with duplicated rules should run: {:?}", result);
}
