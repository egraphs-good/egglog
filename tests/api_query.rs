//! Tests for the typed `EGraph::query` and `EGraph::query_pattern`
//! API. See `src/lib.rs` for `FromRow`, `EGraph::query`, and
//! `EGraph::query_pattern`.

use egglog::prelude::*;
use egglog::{Error, Value};

/// Test 1: `eg.query::<(i64, i64)>("f")` returns all rows of a function
/// table as typed tuples. Order is `(input..., output)`.
#[test]
...
",
    ...
    let mut rows: Vec<(i64, i64)> = egraph.query::<(i64, i64)>("f")?;
    rows.sort();
    assert_eq!(rows, vec![(1, 42), (2, 43), (7, 100)]);
    Ok(())
}

/// `query::<Vec<Value>>` is the escape hatch — useful when the schema
/// contains eq-sort or container columns and you want raw `Value`s.
#[test]
...
",
    ...
    let rows: Vec<Vec<Value>> = egraph.query::<Vec<Value>>("g")?;
...
/// Test 2: `query_pattern::<(i64,)>(vars![x: i64], facts![(R x)])`
...
",
    ...
    let mut results: Vec<(i64,)> = egraph.query_pattern::<(i64,)>(vars![x: i64], facts![(R x)])?;
    results.sort();
    assert_eq!(results, vec![(1,), (2,), (7,)]);
    Ok(())
}

/// Test 3: `query_pattern::<()>(vars![], facts![(R 1 2)])`.
///
...
",
    ...
    let hits: Vec<()> = egraph.query_pattern::<()>(vars![], facts![(R 1 2)])?;
    assert_eq!(hits.len(), 1);

    // Fact absent -> empty Vec.
    let misses: Vec<()> = egraph.query_pattern::<()>(vars![], facts![(R 5 5)])?;
...
",
    ...
    let rows: Vec<Vec<Value>> = egraph.query::<Vec<Value>>("Add")?;
...
/// Iterating a relation table (which is sugar for a constructor with a
/// Relations desugar to constructors with a synthetic non-unionable
/// eq-sort. `EGraph::query` strips that synthetic column so users see
/// just the input columns they declared.
#[test]
fn query_relation_strips_synthetic_output() -> Result<(), Error> {
    let mut egraph = EGraph::default();
    egraph.parse_and_run_program(
        None,
        "
...
",
    ...
    // Typed query — just the i64 input. The synthetic column is hidden.
    let mut rows: Vec<(i64,)> = egraph.query::<(i64,)>("R")?;
    rows.sort();
    assert_eq!(rows, vec![(1,), (2,)]);

    // Vec<Value> escape hatch also sees only the user-declared columns.
    let raw: Vec<Vec<Value>> = egraph.query::<Vec<Value>>("R")?;
    for row in &raw {
        assert_eq!(row.len(), 1, "relation row should expose only inputs");
    }
    Ok(())
}
...
",
    ...
    let mut rows: Vec<(i64, i64)> =
        egraph.query_pattern::<(i64, i64)>(vars![x: i64, y: i64], facts![(= (f x) y)])?;
    rows.sort();
...
fn query_empty_table() -> Result<(), Error> {
    ...
    let rows: Vec<(i64, i64)> = egraph.query::<(i64, i64)>("h")?;
...
fn query_missing_table_errors() {
    ...
    let err = egraph.query::<(i64, i64)>("nonexistent").unwrap_err();
    let msg = format!("{err}");
    assert!(
...
/// through `EGraph::query_pattern::<Vec<Value>>` internally now).
#[test]
#[allow(deprecated)]
