...
/// `egraph.query(vars![x: i64], facts![(R x)].unwrap())` matches and binds `x`
...
",
    ...
    let mut results: Vec<i64> = egraph
        .query(vars![x: i64], facts![(R x)].unwrap())?
        .into_iter()
...
/// `egraph.query(vars![], facts![(R 1 2)].unwrap())` — zero-var case.
///
...
",
    ...
    let hits = egraph.query(vars![], facts![(R 1 2)].unwrap())?;
    assert_eq!(hits.len(), 1);

    let misses = egraph.query(vars![], facts![(R 5 5)].unwrap())?;
...
",
    ...
    let mut rows: Vec<(i64, i64)> = egraph
        .query(vars![x: i64, y: i64], facts![(= (f x) y)].unwrap())?
        .into_iter()
        .map(|m| {
            (
