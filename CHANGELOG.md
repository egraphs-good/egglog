...
- **Container support in the term/proof encoding.** Programs using container sorts (`Vec`, `Set`, `Map`, `MultiSet`, `Pair`) now participate in the term encoding and proof production, where they were previously rejected. During rebuilding, a per-container rebuild primitive canonicalizes a container's eq-sort elements to their union-find leaders, recursing through nested containers (e.g. `(Vec (Vec Math))`); in proof mode it produces a `Congr`-chain proof that the original container equals the rebuilt one, anchored on a per-sort `<Sort>Proof` table (with a reflexive anchor recorded on each rebuilt container so it can be rebuilt again). Container constructor primitives (`vec-of`, `pair`, `set-of`, `multiset-of`, `map-insert`, …) gained proof validators. New `ContainerValues::rebuild_val_with` rebuilds one container against an explicit value remapping. Containers whose canonical form reorders/merges elements (`Set`, `Map`, `MultiSet`) are handled by a new **`ContainerAxiom`** proof justification: the rebuild's `Congr` chain produces the (possibly non-canonical) term, and the axiom normalizes it — sort + dedup for sets, sort for multisets, sort + last-write-wins for maps — to match the canonical value. Normalization is one shared `TermDag` operation (`TermDag::normalize_container_term`, ordered by a new deterministic structural `TermDag::ast_cmp`) used by `reconstruct_termdag`, the container constructor validators, and the proof checker, so all three agree on the canonical term. Consequently container terms now **extract in deterministic AST order** rather than value-id order, and **maps extract in a flat `(map-of k0 v0 k1 v1 …)` form** (a new `map-of` constructor, analogous to `set-of`/`vec-of`) instead of nested `map-insert`s — which also makes map rebuild proofs (including *key collapse*, last-write-wins) check. All container rebuilds — Vec, Pair, Set (collapse + reorder), Map (collapse), MultiSet (count merge) — now produce checkable proofs. The rebuild primitives' configuration is serialized onto the container `(sort …)` declaration as an `:internal-container-rebuild` annotation, so the primitives are re-registered when the encoded program is re-parsed; the proof-encoding desugar round-trip therefore works for container programs.
