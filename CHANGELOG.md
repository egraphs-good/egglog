...
- **`:naive` rule option.** Individual rules can opt out of seminaive evaluation with `:naive` (e.g. `(rule (...) (...) :ruleset r :naive)`). The query and actions then typecheck under the permissive `Read` / `Full` contexts so primitives that read or write the database — including HOFs that wrap custom-function lookups — and direct function-table lookups can run in the rule's RHS. The rule is matched against the entire database every iteration. Use this when correctness depends on reading e-graph state from inside a query and the seminaive trade-offs (untracked dependencies, missed re-firings) are unacceptable.
- **`:unsafe-seminaive` rule option.** Keeps seminaive (delta) evaluation but compiles the rule under the permissive `Read` / `Full` contexts and skips the "no function lookups in actions" check, so the RHS can perform arbitrary database reads — including function-table lookups — without paying for `:naive`'s whole-database matching. It is **unsafe**: a read on a seminaive RHS observes the database mid-iteration (non-monotonic), so results can depend on evaluation order, and `:unsafe-seminaive` rules are rejected by the term/proof encoding. It is mutually exclusive with `:naive`.
