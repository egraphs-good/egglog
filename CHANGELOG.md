...
- **`:naive` rule option.** Individual rules can now opt out of seminaive evaluation with `:naive`, e.g. `(rule (...) (...) :ruleset r :naive)`. The query and the actions are typechecked and dispatched in the global (`GlobalQuery` / `GlobalAction`) contexts, allowing primitives that read or write the database — including higher-order primitives that wrap custom-function lookups — to run inside the rule body. The rule is matched against the entire database every iteration. Use this when a rule's correctness depends on reading e-graph state from inside a query and the unsoundness under seminaive (untracked dependencies, missed re-firings) is acceptable.
