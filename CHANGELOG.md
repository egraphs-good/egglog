...
- **Typed primitive surface for seminaive safety (#772).** Custom primitives now implement `PrimitiveCommon` (name, type-constraint) plus one of four kind-specific traits — `PurePrim`, `WritePrim`, `ReadPrim`, `FullPrim` — each naming the state wrapper its body sees (`PureState<'a, 'db>` / `WriteState<'a, 'db>` / `ReadState<'a, 'db>` / `FullState<'a, 'db>`, exported from `egglog_bridge`). The Rust type checker enforces at compile time that the body only uses methods the chosen state allows; the typechecker enforces that the primitive is only invoked from contexts its state is valid in.
  - Breaking: `impl Primitive for X` (with `type State<'a>` GAT) is replaced by `impl PrimitiveCommon for X` plus `impl <Kind>Prim for X`. State wrappers are two-lifetime (`<'a, 'db>`) and hold `&'a mut ExecutionState<'db>` directly, so primitive method bodies dispatch statically with no `dyn` indirection. The full capability set (pure ops, DB writes, name-indexed `insert`/`remove`/`subsume`/`lookup`/`union`/`panic`) is exposed as inherent methods on each wrapper — no trait imports needed at the call site.
  - `add_primitive` is replaced by `add_pure_primitive` / `add_write_primitive` / `add_read_primitive` / `add_full_primitive`, each taking an `Option<PrimitiveValidator>` as a second argument. The `PrimitiveGroup` builder has matching `add_pure` / `add_write` / `add_read` / `add_full` methods.
  ...
  - The previous `RustRuleContext` is gone; `rust_rule` callbacks now receive a `&mut WriteState` directly. Its name-indexed helpers (`insert` / `remove` / `subsume` / `lookup` / `union` / `panic`) and the base/container conversion sugar (`value_to_base`, `base_to_value`, `value_to_container`, `container_to_value`) are inherent methods on the relevant wrappers, looked up against a shared `egglog_bridge::ActionRegistry` owned by the bridge `EGraph`.
  - Context-aware primitive dispatch moved into the typechecker. `Context` is threaded through constraint generation and the `annotate_*` pass; the constraint builder filters branches by `valid_contexts` and the rule builder uses the typechecker's choice directly. To register multiple variants of the same logical primitive (e.g. `unstable-app`'s pure/full pair), use the new [`EGraph::add_primitive_group`] API — variants registered together have their `valid_contexts` **partitioned into disjoint context sets** (most specific claims first), so each call site has at most one matching variant and the constraint solver's XOR resolves cleanly without grouping or picker machinery.
  - Crate-root rustdoc gained an "Extending egglog from Rust" section linking to the four `*Prim` traits, `prelude::rust_rule`, and the sort traits. Each kind trait's docs spell out the state→context mapping. The error message for using a primitive in a forbidden context lists the contexts where it IS valid.
