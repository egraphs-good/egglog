...
- **`:naive` rule option.** Individual rules can now opt out of seminaive evaluation with `:naive`, e.g. `(rule (...) (...) :ruleset r :naive)`. The query and the actions are typechecked and dispatched in the `Read` and `Full` contexts respectively, allowing primitives that read or write the database — including higher-order primitives that wrap custom-function lookups — to run inside the rule body. The rule is matched against the entire database every iteration. Use this when a rule's correctness depends on reading e-graph state from inside a query and the unsoundness under seminaive (untracked dependencies, missed re-firings) is acceptable.
- **`Context` variants renamed for consistency with the state wrappers.** `RuleQuery → Pure`, `RuleAction → Write`, `GlobalQuery → Read`, `GlobalAction → Full`. Each variant now matches the state wrapper that's required for primitives admitted in that context (`Pure` ↔ `PureState`, etc.). External code referencing `Context::RuleQuery` etc. must rename accordingly.
