...
- **Schedulers now receive a `SchedulerContext`.** `Scheduler::filter_matches` and `can_stop` receive the current e-node count, the only read access needed by size-aware scheduling. Add `EGraph::num_nodes` for the same constructor-node measure.
