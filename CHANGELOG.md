...
- Fix custom schedulers so held matches are rechecked against the current rule body before actions run, preventing persistent scheduler keys made stale by subsumption or deletion from firing while letting fresh matches skip validation.
