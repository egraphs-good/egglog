...
- **Quasiquote macros: write egglog inline in Rust (#947).** The `egglog-quote` crate — re-exported through `egglog::prelude` — lets you write egglog as ordinary Rust tokens instead of a string, splicing Rust values in (`#x`, `#..xs`, `:#field`), with a parse error pointing back at your Rust source rather than into a string literal. Every form has a parse macro — `egglog!` (a whole program), `expr!`, `query!`, `command!`, `rule!`, `action!`/`actions!` — plus `resolve_*!` (typecheck against an e-graph, no run) and `run_*!` (execute against an e-graph) variants; `sexp!`/`sexps!` build un-parsed fragments to splice into another quasiquote. See the `egglog::prelude` module docs for the full guide. Migration: the old `facts!`/`fact!`/`datatype!` macros and the `rule`, `add_sort`, `add_function`, `add_constructor`, `add_relation` prelude helpers are removed — use `query!` and `run_egglog!(egraph, …)`; `add_base_sort` / `add_container_sort` no longer take a `Span`.
- **Compile-time-checked egglog (new `egglog-checked` crate).** `egglog_checked!` and `run_egglog_checked!` type-check a fully-known egglog program *while your crate compiles* — a parse or type error becomes a build error at the macro call site — yielding `Vec<Command>` or a populated `EGraph` respectively. `egglog_header!` declares a reusable schema, checked once where it is defined, so egglog split across crates can be checked against it via `egglog_checked!(schema; …)`.
