...
- **Breaking:** a function's signature is stored once, shared between `TypeInfo`, `Function`, and `GenericFunctionDecl` instead of copied into each. `Function::schema() -> &ResolvedSchema` becomes `Function::func_type() -> &FuncType` and `ResolvedSchema` is removed (its `get_by_pos` moves to `FuncType`); `GenericFunctionDecl::resolved_schema` becomes `Option<Arc<FuncType>>`, so an unresolved decl holds `None` rather than a placeholder `String`.
