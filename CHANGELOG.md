...
- **The scheduler runner applies each rule's chosen matches immediately.** After a rule's `filter_matches`, its chosen matches are applied (flush + action rule) before the next rule's `filter_matches` is called, instead of batching every rule's matches into one apply at the end of the iteration, so schedulers observe up-to-date e-graph sizes within an iteration. The e-graph is still rebuilt once per iteration (egglog-bridge gains `run_rules_no_rebuild`, `flush_updates_no_rebuild`, and `rebuild_now` for this). Measured on Herbie's rewriting workload with identical scheduling decisions, the per-rule application costs nothing single-threaded and 0–7 % wall time with 8 threads.
- `Read::is_sort_unionable` lets read primitives tell e-node tables (constructors over unionable eq-sorts) from fact tables (`relation`s, base-sort functions) the way `EGraph::num_nodes` does.
