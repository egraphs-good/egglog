...
- **Name-indexed e-graph access from primitives and `rust_rule` callbacks (#745, #751).** New `Read` / `Write` capability traits on the state wrappers let primitive bodies and rule callbacks read/write tables by name (`fs.lookup("f", k)`, `fs.set("g", k, v)`, `fs.add("Cons", inputs)`, `fs.union(x, y)`, `fs.eclass_of(...)`, `fs.function_entries(...)`, `fs.constructor_enodes(...)`, etc.) instead of going through raw `FunctionId` + `&[Value]`. `EGraph::update(|fs| ...)` gives the same surface outside a rule (flushes once after the closure). Misuse (wrong subtype, wrong arity, unknown table) surfaces as `Error::ApiError`; per-column sort matching is not checked.
