...
- **Breaking:** a function's signature is stored once, shared between `TypeInfo` and `Function` instead of copied into both. `Function::schema() -> &ResolvedSchema` becomes `Function::func_type() -> &FuncType`, and `ResolvedSchema` is removed (its `get_by_pos` moves to `FuncType`).
