...
- **Typed primitive surface for seminaive safety (#772).** Custom primitives now implement `PrimitiveCommon` (name, type-constraint) plus one of four kind-specific traits — `RuleQueryPrim`, `RuleActionPrim`, `GlobalQueryPrim`, `GlobalActionPrim` — each naming the state wrapper its body sees (`RuleQueryState<'a, 'db>` / `RuleActionState<'a, 'db>` / etc., exported from `egglog_bridge`). The Rust type checker enforces at compile time that the body only uses methods the chosen state allows; the typechecker enforces that the primitive is only invoked from contexts its state is valid in. The wrappers gate database reads and writes via `ExecStateCore` / `ExecStateWriteDb` traits.
  - Breaking: `impl Primitive for X` (with `type State<'a>` GAT) is replaced by `impl PrimitiveCommon for X` plus `impl <Kind>Prim for X`. State wrappers are now two-lifetime (`<'a, 'db>`) — they hold `&'a mut ExecutionState<'db>` directly, so primitive method bodies dispatch statically with no `dyn` indirection.
  - `add_primitive` is replaced by `add_rule_query_primitive` / `add_rule_action_primitive` / `add_global_query_primitive` / `add_global_action_primitive` (and `..._with_validator` variants). The `PrimitiveGroup` builder gets matching `add_rule_query` / `add_rule_action` / `add_global_query` / `add_global_action` methods.
  ...
  - Context-aware primitive dispatch moved into the typechecker. `Context` is now threaded through constraint generation and the `annotate_*` pass; the constraint builder filters branches by `valid_contexts` and the rule builder uses the typechecker's choice directly. To register multiple variants of the same logical primitive (e.g. `unstable-app`'s pure/full pair), use the new [`EGraph::add_primitive_group`] API — variants registered together have their `valid_contexts` **partitioned into disjoint context sets** (most specific claims first), so each call site has at most one matching variant and the constraint solver's XOR resolves cleanly without grouping or picker machinery. `PrimitiveWithId::dedup_key`, `add_primitive_with_dedup_key`, and `TypeConstraint::signature_key` are gone — they were a context-blind workaround for the same problem.
