...
pub trait Cost: Copy + PartialOrd + PartialEq + Debug + Ord {
    fn zero() -> Self;
    fn one() -> Self;
    fn add(&self, other: &Self) -> Self;
}

macro_rules! impl_num_cost {
    ($t:ty) => {
        impl Cost for $t {
            fn zero() -> Self {
                num::Zero::zero()
...
pub type DefaultCost = NotNan<f64>;
...
impl CostModel<DefaultCost> for TreeAdditiveCostModel {
    ...
    fn enode_cost(
        ...
        func.decl.cost.unwrap_or(DefaultCost::one())
    }
}
