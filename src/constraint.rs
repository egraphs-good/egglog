use crate::{
    ...
    core::{
        Atom, AtomTerm, CoreAction, CoreRule, GenericCoreActions, Query, ResolvedCall, StringOrEq,
    },
    literal_sort,
    sort::I64Sort,
...
impl Assignment<AtomTerm, ArcSort> {
    pub(crate) fn annotate_expr(
        &self,
        expr: &GenericExpr<CorrespondingVar<String, String>, String, Literal>,
        typeinfo: &TypeInfo,
    ) -> ResolvedExpr {
        match &expr {
            GenericExpr::Lit(span, literal) => ResolvedExpr::Lit(
                span.clone(),
            ...
            GenericExpr::Var(span, var) => {
                ...
                let ty = global_ty
                    ...
                    .or_else(|| {
                        self.get(&AtomTerm::Var(DUMMY_SPAN.clone(), var.clone()))
                            .cloned()
                    })
            ...
            GenericExpr::Call(
                ...
                let types: Vec<_> = args
                    ...
                    .chain(once(
                        self.get(&AtomTerm::Var(
                            DUMMY_SPAN.clone(),
                            corresponding_var.clone(),
                        ))
                        .unwrap()
                        .clone(),
                    ))
    ...
    pub(crate) fn annotate_fact(
        &self,
        facts: &GenericFact<CorrespondingVar<String, String>, String, Literal>,
        typeinfo: &TypeInfo,
    ) -> ResolvedFact {
        match facts {
    ...
    pub(crate) fn annotate_facts(
        &self,
        mapped_facts: &[GenericFact<CorrespondingVar<String, String>, String, Literal>],
        typeinfo: &TypeInfo,
    ) -> Vec<ResolvedFact> {
        mapped_facts
            .iter()
    ...
    pub(crate) fn annotate_actions(
        &self,
        mapped_actions: &GenericActions<CorrespondingVar<String, String>, String, Literal>,
        typeinfo: &TypeInfo,
...
impl Problem<AtomTerm, ArcSort> {
    pub(crate) fn add_query(
        &mut self,
        query: &Query<StringOrEq, String, Literal>,
        typeinfo: &TypeInfo,
    ) -> Result<(), TypeError> {
    ...
    pub fn add_actions(
        &mut self,
        actions: &GenericCoreActions<String, String, Literal>,
        typeinfo: &TypeInfo,
    ...
    pub(crate) fn get_constraints(
        ...
        match self {
            ...
            CoreAction::Change(span, _change, head, args) => {
                ...
                let var = symbol_gen.fresh(&head.to_string());
...
impl Atom<StringOrEq> {
    pub fn get_constraints(
        ...
        match &self.head {
            ...
            StringOrEq::String(head) => Ok(literal_constraints
                .chain(get_atom_application_constraints(
...
fn get_atom_application_constraints(
    ...
    match xor_constraints.len() {
        0 => Err(TypeError::UnboundFunction(head.clone(), span.clone())),
...
impl SimpleTypeConstraint {
    pub fn new(name: String, sorts: Vec<ArcSort>, span: Span) -> SimpleTypeConstraint {
...
impl AllEqualTypeConstraint {
    pub fn new(name: String, span: Span) -> AllEqualTypeConstraint {
