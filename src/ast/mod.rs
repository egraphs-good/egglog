    ...
    Leaf: Clone + PartialEq + Eq + Display + Hash,
{
    ...
    /// ```
    ///
    /// A constrctor can also have a `cost` for extraction, which is used in the cost model for extraction. It defaults to 1 if not specified.
    /// ```text
    /// (constructor ExpensiveNode (i64) Math :cost 10)
    /// ```
    ///
    /// Finally, a constructor can be `:hidden`, which is even stronger than `:unextractable`. Hidden nodes are excluded even from print-size output, and are currently used for internal tables generated by proof production.
    /// ```text
    /// (constructor HiddenNode (i64) Math :hidden)
    /// ```
    Constructor {
        span: Span,
        name: String,
        ...
        unextractable: bool,
        /// Hidden constructors are excluded from print-size output.
