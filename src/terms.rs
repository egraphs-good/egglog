...
impl<'a> TermState<'a> {
    ...
    fn make_parent_table(&mut self, name: &String) -> Vec<Command> {
        let pname = self.parent_name(name);
        let to_union_name = self.to_union_name(name);
        self.parse_program(&format!(
            "(relation {pname} ({name} {name}))
             (rule (({pname} a b)
                    ({pname} b c)
                    (!= b c)
                    (!= a c))
    ...
    fn view_name(&self, name: String) -> String {
    ...
    /// can be deleted.
    fn make_term_view(&self, fdecl: &ResolvedFunctionDecl) -> Vec<Command> {
        let schema = &fdecl.schema;
        let mut types = schema.input;
    ...
    fn instrument_rule(&mut self, ruleset: String, name: String, rule: &NormRule) -> Vec<Command> {
        let rule = Rule {
    ...
    fn instrument_schedule(&mut self, schedule: &NSchedule) -> Schedule {
    ...
    pub(crate) fn add_term_encoding_helper(&mut self, program: Vec<NormCommand>) -> Vec<Command> {
        ...
        for command in program {
            ...
            match &command.command {
                ...
                NCommand::Fail(cmd) => {
                    let mut with_term_encoding = self.add_term_encoding_helper(vec![NCommand {
                        command: *cmd.clone(),
    ...
    pub fn parse_program(&self, input: &str) -> Result<Vec<Command>, ParseError> {
        self.egraph.parser.get_program_from_string(input)
    }
}
