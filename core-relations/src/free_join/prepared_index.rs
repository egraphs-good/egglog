...
/// Retained index state for one logical indexed access during a query execution.
///
...
impl PreparedIndexSlot {
    ...
    ) -> Option<&RootProjection> {
        let slot = if let Some(slot) = self.projected_root.get() {
            slot
        } else {
            let candidate = root.projection_slot(column, slow_constraints)?;
            self.projected_root.get_or_init(|| candidate)
        };
