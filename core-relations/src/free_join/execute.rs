    ...
    pub fn run_rule_set(&mut self, rule_set: &RuleSet, report_level: ReportLevel) -> RuleSetReport {
        ...
        if parallelize_db_level_op(self.total_size_estimate) {
            ...
            egglog_concurrency::scope(|scope| {
                for (plan, desc, symbol_map) in rule_set.plans.values() {
                    ...
                    scope.spawn(move |_| {
                        ...
                        let prepared_plan = PreparedPlanIndexes::new(db, plan);
                        let search_and_apply_timer = Instant::now();
            ...
            for (plan, desc, symbol_map) in rule_set.plans.values() {
                ...
                // arena is reclaimed.
                let arena = SharedArena::new();
                let prepared_plan = PreparedPlanIndexes::new(self, plan);
                let report_plan = match report_level {
                    ReportLevel::TimeOnly => None,
                    ReportLevel::WithPlan | ReportLevel::StageInfo => {
                        Some(plan.to_report(symbol_map))
...
/// Choose the physical execution order for some suffix of an already-planned
/// stage block.
///
/// # Static and dynamic variable ordering
///
/// Tree decomposition first decides which variables and atoms belong to each
/// bag; that is separate from the within-bag ordering described here. The
/// planner then compiles every bag, plus a decomposed plan's final result
/// block, into an immutable, cacheable sequence of [`JoinStage`]s. Generic Join
/// visits each useful variable in stable variable id order before fusing
/// compatible scans. Free Join instead orders cover atoms using planning-time
/// size estimates and may bind several variables in one stage. Besides
/// providing a deterministic fallback, this logical order fixes stage
/// identities, cover choices, constraints, and materialization dependencies,
/// none of which dynamic ordering changes.
///
/// At the start of each execution, [`JoinState::run_join_stages`] creates an
/// identity [`InstrOrder`] and calls this function with `start == 0`. We can
/// therefore choose an initial physical order from the live, header-filtered
/// subsets even when the cached plan's estimates are stale, without mutating
/// the plan. Top-level parallel shard selection happens only after this first
/// reorder.
///
/// [`JoinState::run_plan`] may call this function again after earlier stages
/// have produced branch-local residual subsets. Currently it reorders at
/// depths 1, 4, 7, ... when the current scan has more than 32 rows and at
/// least two stages remain. Only positions at or after `start` move, and
/// `FusedIntersectMat` stages in `Lookup`, `Value`, or `Full` mode remain fixed
/// as barriers between independently reorderable phases.
///
/// Each phase is rebuilt greedily. When the entire stage block consists only
/// of `Intersect` stages, candidates are ordered lexicographically by:
///
/// 1. decreasing refinement count in the current physical-order prefix;
/// 2. increasing live residual-size estimate;
/// 3. decreasing refinement count in the cached logical-plan prefix; and
/// 4. decreasing number of relations intersected by the stage.
///
/// At a recursive reorder, the physical count is seeded from the stages
/// actually chosen before the phase's starting position, while the stable
/// count is seeded from the same-length prefix of the cached instruction
/// vector. Both are then extended after each greedy suffix choice. An
/// `Intersect` candidate's refinement count is the maximum count of any atom
/// it scans.
/// The first key therefore favors a continuation connected to the local query
/// frontier and helps avoid accidental products, but it is a preference rather
/// than a hard eligibility rule. Estimated input size decides among equally
/// connected candidates; the logical-prefix count then supplies a
/// canonical-prefix locality bias.
///
/// Mixed/Fused blocks instead use a stable refinement score first, followed by
/// live size and relation count. Intersections and fused covers derive that
/// score from the logical prefix; materialized scans use their binding width.
/// Making the primary signal branch-local creates many divergent suffix orders
/// and probe paths, which is especially costly for fused covers. Size is
/// deliberately only a cheap row-count proxy: an intersection uses its
/// smallest input subset, a fused scan uses its cover subset, and a
/// materialized scan uses its materialization length.
///
/// Reordering can also change which stages qualify for factorized leaf-scan
/// execution, so [`recompute_leaf_scans`] must run after every change.
...
fn sort_plan_by_size_inner(
    ...
    // Seed the locality bias from the stable plan prefix, not the physical
    // prefix chosen for this branch. It remains the primary key for mixed
    // plans and a deterministic locality tie-breaker for all-Intersect blocks.
    ...
    // Mixed/Fused plans prioritize stages by
    //
    ...
    for i in range.clone() {
        ...
        for j in i..range.end {
            ...
            let key = if let Some(physical_refinements) = &physical_refinements {
                // An all-Intersect block follows the locally connected query
                // frontier. Refinement depth biases against accidental products;
                // among equally refined frontier stages, estimated input size
                // gets priority over the stable locality tie-breaker.
                (
                ...
                // Preserve the stable-prefix ordering for mixed/Fused plans:
                // branch-local refinement feedback is particularly harmful
                // for those plans.
...
mod top_index_tests {
    ...
    fn pure_intersect_dvo_prioritizes_physical_refinement_depth() {
        ...
        // The logical-plan prefix favors atom 1, and atom 1 is much smaller.
        // The physical prefix has nevertheless refined atom 0 twice and atom 1
        // once, so the local continuation on atom 0 must run first.
    ...
    fn pure_intersect_dvo_uses_cardinality_at_equal_physical_depth() {
        ...
        let mut binding_info = BindingInfo::default();
        for (atom, size) in [(0, 50), (1, 10), (2, 100)] {
            binding_info.insert_subset(
                AtomId::from_usize(atom),
                Subset::Dense(crate::OffsetRange::new(
                    crate::RowId::from_usize(0),
        ...
        // The stable logical-plan prefix favors atom 0 twice, but the physical
        // prefix has refined atoms 0 and 1 equally. Cardinality therefore
        // selects atom 1.
        let mut order = InstrOrder::from_iter([4, 5, 2, 3, 0, 1].into_iter());
        sort_plan_by_size_inner(&mut order, 2..4, &stages, &mut binding_info);
