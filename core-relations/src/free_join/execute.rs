                    ...
                    scope.spawn(move |rule_scope| {
                        ...
                        let report_plan = match report_level {
                            ...
                            ReportLevel::StageInfo => {
                                let stats =
                                    join_state.stage_stats.as_ref().map(|s| s.collect());
...
impl PlanStageStats {
    ...
    fn add(&self, stage_idx: usize, candidates: usize, succeeded: usize) {
        if stage_idx < self.data.len() {
            self.data[stage_idx].0.fetch_add(candidates, Ordering::Relaxed);
            self.data[stage_idx].1.fetch_add(succeeded, Ordering::Relaxed);
        }
    }
        ...
        'a: 'buf,
    {
        ...
        match &stages.instrs[stage_idx] {
            ...
            JoinStage::FusedIntersect {
                ...
                // All scanned rows pass (constraints are applied in scan_project).
