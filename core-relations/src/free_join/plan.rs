...
pub(crate) fn tree_decompose_and_plan(
    ...
    while let Some((var, subquery_vars)) = next_var_to_eliminate(&vars, &atoms, &bags) {
        ...
        let subquery_atoms = DenseIdMap::from_iter(subquery_atoms.into_iter().map(|atom_id| {
            (atom_id, ctx.atoms[atom_id].clone())
            // let atom_info = atoms.take(atom_id).unwrap();
            // if atom_info.table.is_dummy() {
            //     None
            // } else {
