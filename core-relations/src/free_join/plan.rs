...
fn plan_gj(
    ...
    // Use Kahn's topological sort to order variables while respecting FDs.
    //
    // For each FD (antecedents → consequent) where both the antecedent and
    // consequent appear in the plan, we add a directed edge antecedent →
    // consequent.  Kahn's algorithm then produces an ordering that:
    //   1. Respects all FD constraints (antecedents before consequents).
    //   2. Uses (size, -occs, var_id) as the priority for unordered pairs,
    //      preserving the existing heuristic for variables with no FD relation.
    //
    // Placing antecedents before consequents enables the SparseColumn O(1)
    // fast-path in the executor: once a key is bound, the corresponding value
    // column has exactly one row instead of requiring a full IndexMap build.
    {
        let n = min_sizes.len();
        // Build per-variable info lookup.
        let mut var_info_map: HashMap<Variable, (usize, usize)> = HashMap::default();
        for &(var, size, occs) in &min_sizes {
            var_info_map.insert(var, (size, occs));
        }
        let var_set: HashSet<Variable> = var_info_map.keys().copied().collect();

        // Build adjacency list and in-degree map.
        let mut in_degree: HashMap<Variable, usize> = HashMap::default();
        let mut out_edges: HashMap<Variable, Vec<Variable>> = HashMap::default();
        for &(var, _, _) in &min_sizes {
            in_degree.insert(var, 0);
            out_edges.insert(var, Vec::new());
        }
        let mut added_edges: HashSet<(Variable, Variable)> = HashSet::default();
        for (antecedents, consequents) in ctx.fun_deps.iter_deps() {
            let in_scope_ants: SmallVec<[Variable; 4]> = antecedents
                .iter()
                .filter(|&&v| var_set.contains(&v))
                .copied()
                .collect();
            for &cq in consequents {
                if !var_set.contains(&cq) {
                    continue;
                }
                for &ant in &in_scope_ants {
                    if added_edges.insert((ant, cq)) {
                        out_edges.get_mut(&ant).unwrap().push(cq);
                        *in_degree.get_mut(&cq).unwrap() += 1;
                    }
                }
            }
        }

        // Priority queue: (size, -occs, var_id) → variable. BTreeMap gives
        // us sorted order with O(log n) insert/remove.
        let make_key = |var: Variable, size: usize, occs: usize| -> (usize, i64, u32) {
            (size, -(occs as i64), var.index() as u32)
        };
        let mut ready: BTreeMap<(usize, i64, u32), Variable> = BTreeMap::new();
        for &(var, size, occs) in &min_sizes {
            if in_degree[&var] == 0 {
                ready.insert(make_key(var, size, occs), var);
            }
        }

        let mut result: Vec<(Variable, usize, usize)> = Vec::with_capacity(n);
        while let Some((&key, &var)) = ready.iter().next() {
            ready.remove(&key);
            let (size, occs) = var_info_map[&var];
            result.push((var, size, occs));
            for &cq in &out_edges[&var].clone() {
                let deg = in_degree.get_mut(&cq).unwrap();
                *deg -= 1;
                if *deg == 0 {
                    let (cq_size, cq_occs) = var_info_map[&cq];
                    ready.insert(make_key(cq, cq_size, cq_occs), cq);
                }
            }
        }

        // Handle cycles: append remaining variables in priority order.
        if result.len() < n {
            let processed: HashSet<Variable> = result.iter().map(|(v, _, _)| *v).collect();
            let mut remainder: Vec<(Variable, usize, usize)> = min_sizes
                .iter()
                .filter(|(v, _, _)| !processed.contains(v))
                .copied()
                .collect();
            remainder.sort_by_key(|&(var, size, occs)| make_key(var, size, occs));
