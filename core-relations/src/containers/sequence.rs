...
mod tests {
    ...
    fn independent_predictions_coalesce_during_merge() {
        ...
        let first = db.with_execution_state(None, |state| env.get_or_insert(&key, state));
        let second = db.with_execution_state(None, |state| env.get_or_insert(&key, state));
    ...
    fn reverse_index_is_rebuilt_after_sequence_compaction() {
        let mut db = Database::new();
        let mut env = test_env(&mut db);
        let ids = db.with_execution_state(None, |state| {
            (0..40)
    ...
    fn full_rebuild_marks_a_stable_identity_dirty() {
        ...
        let mut db = Database::new();
        let mut env = test_env(&mut db);
        let id = db.with_execution_state(None, |state| env.get_or_insert_key(&[from], state));
        db.with_execution_state(None, |state| {
            let change = env.merge_table(state);
            assert!(change.added || change.removed);
        });
        ...
        let summary = db.with_execution_state(None, |state| {
    ...
    fn full_rebuild_remaps_an_outer_identity_without_marking_it_dirty() {
        ...
        let mut db = Database::new();
        let mut env = test_env(&mut db);
        let old_id = db.with_execution_state(None, |state| env.get_or_insert_key(&[child], state));
        db.with_execution_state(None, |state| {
            let change = env.merge_table(state);
            assert!(change.added || change.removed);
        });
